!(function () {
	...
	class G {
		...
		async getAll() {
			return await this._queueDb.getAllEntriesByQueueName(
				this._queueName,
			);
		}
		async size() {
			return await this._queueDb.getEntryCountByQueueName(
				this._queueName,
			);
		}
		async deleteEntry(e) {
		...
		addCacheListener() {
			self.addEventListener('message', (e) => {
				if (e.data && 'CACHE_URLS' === e.data.type) {
					...
					e.waitUntil(a),
						e.ports?.[0] &&
							a.then(() => e.ports[0].postMessage(!0));
				}
			});
		}
	...
	class ec {
		...
		async cachePut(e, t) {
			...
			if (!t)
				throw new d('cache-put-with-no-response', { url: b(s.url) });
			...
			try {
				...
				if (i instanceof Error) {
					for (let n of e.iterateCallbacks('handlerDidError'))
						if (
							void 0 !==
							(s = await n({ error: i, event: a, request: t }))
						)
							break;
				}
				if (!s) throw i;
	...
	class ef extends el {
		...
		async _handle(e, t) {
			...
			let r = await t.waitUntil(
				(async () =>
					(await t.waitUntil(Promise.race(i))) || (await n))(),
			);
	...
	let eg = 'www.google-analytics.com',
		...
		ev = (e) =>
			new et(
				({ url: e }) =>
					e.hostname === eg && '/analytics.js' === e.pathname,
				new ef({ cacheName: e }),
				'GET',
			),
		ex = (e) =>
	...
	class eS extends el {
		...
		async _handleFetch(e, t) {
			...
			if (this._fallbackToNetwork) {
				...
				(a = await t.fetch(
					new Request(e, {
						integrity: 'no-cors' !== e.mode ? n || i : void 0,
					}),
				)),
					i &&
						r &&
...
This is generally NOT safe. Learn more at https://bit.ly/wb-precache`);
		...
		activate(e) {
			return R(e, async () => {
				let e = await self.caches.open(this.strategy.cacheName),
					...
					s = [];
				for (let i of t)
					a.has(i.url) || (await e.delete(i), s.push(i.url));
	...
	let eT = '-precache-',
		eA = async (e, t = eT) => {
			let a = (await self.caches.keys()).filter(
				(a) =>
					a.includes(t) &&
					a.includes(self.registration.scope) &&
					a !== e,
			);
		...
		async notifyIfUpdated(e) {
			if (
				...
				if ('navigate' === e.request.mode) {
					let t;
					e.event instanceof FetchEvent &&
						(t = e.event.resultingClientId),
						(!(await x(t)) || eF) && (await v(3500));
				}
				if (this._notifyAllClients)
					for (let e of await self.clients.matchAll({
						type: 'window',
					}))
						e.postMessage(t);
	...
	class eV {
		...
		constructor(e = {}) {
			(this._config = e),
				...
				e.purgeOnQuotaError &&
					f.add(() => this.deleteCacheAndMetadata());
		}
		_getCacheExpiration(e) {
			if (e === h.getRuntimeName())
				throw new d('expire-custom-caches-only');
			let t = this._cacheExpirations.get(e);
			return (
				t ||
					((t = new eG(e, this._config)),
		...
		try {
			...
			let s = (function (e) {
					...
					if (!t.startsWith('bytes='))
						throw new d('unit-must-be-bytes', {
							normalizedRangeHeader: t,
						});
					if (t.includes(','))
						throw new d('single-range-only', {
							normalizedRangeHeader: t,
						});
					let a = /(\d*)-(\d*)/.exec(t);
					if (!a || !(a[1] || a[2]))
						throw new d('invalid-range-values', {
							normalizedRangeHeader: t,
						});
					return {
				...
				n = (function (e, t, a) {
					...
					let n = e.size;
					if ((a && a > n) || (t && t < 0))
						throw new d('range-not-satisfiable', {
							size: n,
							end: a,
							start: t,
						});
	...
	let eJ = () => {
			...
			(e = e.map(
				(e) => (
					!e.options ||
						...
						e.options.plugins?.some(
							(e) => 'handlerDidError' in e,
						) ||
		...
		eY = ({
			...
			if (
				...
				er(
					new ea(
						((i = s.navigateFallback),
						eN().createHandlerBoundToURL(i)),
						{
	...
	(({
		...
		precacheEntries: [
			{
				revision: '90ae5ad451b1cd81',
				url: '/_next/static/chunks/664-90ae5ad451b1cd81.js',
			},
			{
				revision: 'd0154717f77ffdb3',
				url: '/_next/static/chunks/framework-d0154717f77ffdb3.js',
			},
			{
				revision: '7566dbc591c362fc',
				url: '/_next/static/chunks/main-7566dbc591c362fc.js',
			},
			{
				revision: '470d39536efdd47c',
				url: '/_next/static/chunks/pages/_app-470d39536efdd47c.js',
			},
			{
			...
			{
				revision: '8c1b0cf90d3d0149',
				url: '/_next/static/chunks/pages/auth/signin-8c1b0cf90d3d0149.js',
			},
			{
				revision: 'ab5f150172f5cb29',
				url: '/_next/static/chunks/pages/create/routine-ab5f150172f5cb29.js',
			},
			{
				revision: '0576aa96f83e13c8',
				url: '/_next/static/chunks/pages/create/session-0576aa96f83e13c8.js',
			},
			{
				revision: '9f453fa1866a4e42',
				url: '/_next/static/chunks/pages/create/workout-9f453fa1866a4e42.js',
			},
			{
				revision: '89f29f15f083c162',
				url: '/_next/static/chunks/pages/home-89f29f15f083c162.js',
			},
			{
				revision: '4dc0012fb8b6a3a1',
				url: '/_next/static/chunks/pages/index-4dc0012fb8b6a3a1.js',
			},
			{
				revision: '9a20b57182e17a84',
				url: '/_next/static/chunks/pages/manage-9a20b57182e17a84.js',
			},
			{
				revision: '71431e15bdf23735',
				url: '/_next/static/chunks/pages/manage/routines-71431e15bdf23735.js',
			},
			{
				revision: 'd71fefe274861f98',
				url: '/_next/static/chunks/pages/manage/sessions-d71fefe274861f98.js',
			},
			{
				revision: '0dd03a2bb8091bdb',
				url: '/_next/static/chunks/pages/manage/workouts-0dd03a2bb8091bdb.js',
			},
			{
				revision: '9fc46cc950cc582c',
				url: '/_next/static/chunks/pages/settings-9fc46cc950cc582c.js',
			},
			{
				revision: 'd4b7b07bb0cfbdae',
				url: '/_next/static/chunks/pages/settings/account-d4b7b07bb0cfbdae.js',
			},
			{
				revision: '851f3e2bdb5df856',
				url: '/_next/static/chunks/pages/signup-851f3e2bdb5df856.js',
			},
			{
			...
			{
				revision: 'c0918c84c37ac298',
				url: '/_next/static/chunks/webpack-c0918c84c37ac298.js',
			},
			{
				revision: 'ac9611df2bd71413',
				url: '/_next/static/css/ac9611df2bd71413.css',
			},
			{
				revision: 'ecac83fea96a8c244f93ab103363476f',
				url: '/_next/static/tE3I-3JU7ULM5bt_iYTWx/_buildManifest.js',
			},
			{
				revision: 'b6652df95db52feb4daf4eca35380933',
				url: '/_next/static/tE3I-3JU7ULM5bt_iYTWx/_ssgManifest.js',
			},
			{
				revision: 'ab6f6617e972d34628a856c322684918',
				url: '/favicon.ico',
			},
			{
				revision: 'd3d88bd514274ef062d7855b3955d210',
				url: '/flexed_biceps_3d_default.png',
			},
			{
				revision: '014f0c3d020ce3feda8792d58ff65aff',
				url: '/manifest.json',
			},
			{ revision: '71782f0a19e1ac2b7126fa129afe790b', url: '/sw.js' },
			{
				revision: 'ef691b5c1e5be7ac9fd2af5a22a5bbd5',
				url: '/workbox-e43f5367.js',
			},
			{
				revision: '0c3b49c61954210e95d72cea5748bbec',
